...
if(isset($_POST['act_type']) && $_POST['act_type'] == 'usa-html5-map-states-save') {
    ...
    if ($sId) {
        ...
        if(isset($_POST['URL'][$vals['id']]))
            $states[$s_id]['link'] = usa_html5map_plugin_chk_url(stripslashes($_POST['URL'][$vals['id']]));
        ...
        if(isset($_POST['color'][$vals['id']]))
            $states[$s_id]['color_map'] = usa_html5map_plugin_chk_color($_POST['color'][$vals['id']]);

        if(isset($_POST['color_'][$vals['id']]))
            $states[$s_id]['color_map_over'] = usa_html5map_plugin_chk_color($_POST['color_'][$vals['id']]);
        ...
        foreach($states as $k=>$v) {
            $states[$k]['color_map'] = usa_html5map_plugin_chk_color($_POST['color'][$vals['id']]);
        }
        }

        if(isset($_POST['colorOverCh'][$vals['id']])) {
        foreach($states as $k=>$v) {
            $states[$k]['color_map_over'] = usa_html5map_plugin_chk_color($_POST['color_'][$vals['id']]);
        }
        }
...
echo "<div class=\"wrap usa-html5-map main full\"><h2>" . __('Configuration of Map Areas', 'usa-html5-map') . "</h2>";
?>
<script>
    var map_cfg = {
...
?>
    ...
    <button type="button" class="button button-secondary map-show" style="margin-top: 1px" data-toggle="<?php esc_attr_e('Hide map', 'usa-html5-map'); ?>"><?php _e('Choose on map', 'usa-html5-map') ?></button>

    <button class="button button-secondary bulk-selectall notbulk-hide" style="margin-top: 1px"><?php _e('Toggle selections', 'usa-html5-map') ?></button>

    <?php if ($isbulk) { ?>
        <a href="admin.php?page=usa-html5-map-states&map_id=<?php echo $map_id; ?>" class="button button-secondary bulk-editing-classic" style="margin-top: 1px"><?php _e('Back to one-by-one editing', 'usa-html5-map') ?></a>
    <?php } else { ?>
        <a href="admin.php?page=usa-html5-map-states&map_id=<?php echo $map_id; ?>&bulk" class="button button-secondary bulk-editing bulk-hide" style="margin-top: 1px"><?php _e('Bulk Edit', 'usa-html5-map') ?></a>
    <?php } ?>
    ...
    <fieldset>
        ...
        <?php usa_html5map_plugin_wp_editor_for_tooltip($vals['comment'], "info[{$vals['id']}]", 'info'); ?>
        <br />
        ...
        <label style="margin-left:20px; cursor: default"><span style="color: #666">ID: <?php echo 'st'.$sId ?> </span><span class="tipsy-q" original-title="<?php esc_attr_e('Use this ID when interacting with the map via API', 'usa-html5-map'); ?>">[?]</span></label>
        <br />

        <span class="title"><?php echo __('Hide area:', 'usa-html5-map'); ?> </span>
        <label><input name="hidden[<?php echo $vals['id']?>]" type="radio" value="show" <?php if (!isset($vals['hidden'])) echo 'checked="checked"'; ?> /> <?php echo __('show', 'usa-html5-map'); ?></label>
        <label><input name="hidden[<?php echo $vals['id']?>]" type="radio" value="hide" <?php if (isset($vals['hidden'])) echo 'checked="checked"'; ?> /> <?php echo __('hide', 'usa-html5-map'); ?></label>
        <br />
        </div>
