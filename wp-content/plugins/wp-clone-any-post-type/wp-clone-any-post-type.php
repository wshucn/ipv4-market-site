...
class GwlCloneAnyPostType {
	...
	public function __construct() {
		...
		add_filter( 'plugin_row_meta', array( &$this,'wcapt_add_custom_plugin_links') , 10, 2);
		add_filter( 'plugin_action_links', array( &$this,'wcapt_settings_link'), 10, 2 );
	}

	/*
	*  Plugin Setting Link 
	*/
	public function wcapt_settings_link( $links, $plugin_file ) {
		if( plugin_basename( __FILE__ ) == $plugin_file ){
			$settings_ui_links = sprintf( '<a href="%s">%s</a>', admin_url( 'admin.php?page=wcapt_clone_settings' ), __( 'Settings', 'wp-clone-any-post-type' ) );
	 ...
	 * @return $links [array]
	 */
	function wcapt_add_custom_plugin_links($links, $file) {
		if ($file === 'wp-clone-any-post-type/wp-clone-any-post-type.php') {
			$links[] = '<a href="https://wp-plugins.galaxyweblinks.com/wp-plugins/wp-clone-any-post-type/doc/" target="_blank">Documentation</a>';
