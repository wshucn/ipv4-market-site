...
class Nelio_AB_Testing_Php_Editor_Page {
	...
	public function init() {
		...
		add_submenu_page( '__nelio-ab-testing', '', '', 'edit_nab_experiments', 'nelio-ab-testing-php-editor', '' );
	}//end init()
	...
	public function extract_params_from_url_or_die() {
		...
		if ( empty( $alternative ) ) {
			wp_die( esc_html_x( 'You attempted to edit a PHP variant that doesn’t exist. Perhaps it was deleted?', 'user', 'nelio-ab-testing' ) );
		}//end if

		if ( 'nab/php' !== $experiment->get_type() ) {
			wp_die( esc_html_x( 'Test variant is not a PHP variant.', 'user', 'nelio-ab-testing' ) );
		}//end if
	...
	public function enqueue_assets() {
		...
		( function() {
			wp.domReady( function() {
				nab.initPhpEditorPage( "nab-php-editor", %s );
			} );
		} )();';
		...
		wp_enqueue_script( 'nab-php-experiment-admin' );
		...
		wp_enqueue_style( 'nab-php-experiment-admin' );
	}//end enqueue_assets()
	...
	private function is_current_screen_this_page() {
		...
		return 'nelio-ab-testing-php-editor' === sanitize_text_field( wp_unslash( $_GET['page'] ) ); // phpcs:ignore
