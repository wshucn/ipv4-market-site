...
jQuery('.formatting-rule-cell-value.wdt-datepicker').on('dp.hide',function(e){e.stopImmediatePropagation();if(jQuery.inArray(jQuery(this).closest('.wdt-conditional-formatting-rule').find('.formatting-rule-cell-value').val(),['%LAST_WEEK%','%THIS_WEEK%','%NEXT_WEEK%','%LAST_30_DAYS%','%LAST_MONTH%','%NEXT_MONTH%','%THIS_MONTH%'])===-1){jQuery(this).closest('.wdt-conditional-formatting-rule').find('.formatting-rule-if-clause').prop('disabled',!1).wdtBootstrapSelectPicker('val','lt')}else{jQuery(this).closest('.wdt-conditional-formatting-rule').find('.formatting-rule-if-clause').prop('disabled',!0).wdtBootstrapSelectPicker('val','')}});jQuery('.formatting-rule-cell-value.wdt-datetimepicker').on('dp.hide',function(e){e.stopImmediatePropagation();if(jQuery.inArray(jQuery(this).closest('.wdt-conditional-formatting-rule').find('.formatting-rule-cell-value').val(),['%LAST_WEEK%','%THIS_WEEK%','%NEXT_WEEK%','%LAST_30_DAYS%','%LAST_MONTH%','%NEXT_MONTH%','%THIS_MONTH%'])===-1){jQuery(this).closest('.wdt-conditional-formatting-rule').find('.formatting-rule-if-clause').prop('disabled',!1).wdtBootstrapSelectPicker('val','lt')}else{jQuery(this).closest('.wdt-conditional-formatting-rule').find('.formatting-rule-if-clause').prop('disabled',!0).wdtBootstrapSelectPicker('val','')}})}}
...
if($.inArray(tableDescription.currentSkin,['raspberry-cream','mojito','dark-mojito'])!==-1){dataTableOptions.fnInitComplete=function(){let columnPos=tableDescription.dataTableParams.order[0][0];let columnTitle=tableDescription.dataTableParams.columnDefs[columnPos].className.substring(tableDescription.dataTableParams.columnDefs[columnPos].className.indexOf("column-")+7,);let tableId=tableDescription.tableId;addOutlineBorder(tableId,columnTitle);if($.inArray(tableDescription.currentSkin,['mojito','dark-mojito'])!==-1){cubeLoaderMojito(tableId);if(tableDescription.showRowsPerPage)
...
if(tableDescription.pagination&&tableDescription.table_wcag){$(tableDescription.selector+'_paginate').addClass('wcag_paginate');wpDataTables[tableDescription.tableId].fnSettings().oLanguage.oPaginate.sFirst=wpdatatables_frontend_strings.firstPageWCAG;wpDataTables[tableDescription.tableId].fnSettings().oLanguage.oPaginate.sLast=wpdatatables_frontend_strings.lastPageWCAG;wpDataTables[tableDescription.tableId].fnSettings().oLanguage.oPaginate.sNext=wpdatatables_frontend_strings.nextPageWCAG;wpDataTables[tableDescription.tableId].fnSettings().oLanguage.oPaginate.sPrevious=wpdatatables_frontend_strings.previousPageWCAG;$(document).ready(function(){$('#'+tableDescription.tableId+'_paginate').find('span .paginate_button').each(function(index){$(this).attr('aria-label',wpdatatables_frontend_strings.pageWCAG+this.text)})})}
$(document).ready(function(){if(tableDescription.table_wcag){$(tableDescription.selector+'_wrapper .dt-buttons .dt-button.DTTT_button_spacer').attr('aria-label',wpdatatables_frontend_strings.spacerWCAG).attr('role','button');$(tableDescription.selector+'_wrapper .dt-buttons .dt-button.DTTT_button_colvis').attr('aria-label',wpdatatables_frontend_strings.colvisWCAG).attr('role','button');$(tableDescription.selector+'_wrapper .dt-buttons .dt-button.DTTT_button_print').attr('aria-label',wpdatatables_frontend_strings.printTableWCAG).attr('role','button');$(tableDescription.selector+'_wrapper .dt-buttons .dt-button.DTTT_button_export').attr('aria-label',wpdatatables_frontend_strings.exportTableWCAG).attr('role','button');$(tableDescription.selector+'_wrapper .dt-buttons .dt-button.DTTT_button_clear_filters').attr('aria-label',wpdatatables_frontend_strings.clearFiltersWCAG).attr('role','button')}});wpDataTables[tableDescription.tableId].fnSettings().aoDrawCallback.push({sName:'removePaginate',fn:function(oSettings){var api=oSettings.oInstance.api();if(typeof(api.page.info())!='undefined'){if(api.page.len()>=api.page.info().recordsDisplay||api.data().page.len()==-1){$('#'+tableDescription.tableId+'_paginate').hide()}else{$('#'+tableDescription.tableId+'_paginate').show()}}}});wpDataTables[tableDescription.tableId].fnSettings().aoDrawCallback.push({sName:'addOutlineClass',fn:function(oSettings){if($.inArray(tableDescription.currentSkin,['raspberry-cream','mojito','dark-mojito'])!==-1){let columnPos=oSettings.aaSorting[0][0];let columnTitle=oSettings.aoColumns[columnPos].className.substring(oSettings.aoColumns[columnPos].className.indexOf("column-")+7,);let tableId=oSettings.sTableId;addOutlineBorder(tableId,columnTitle)}}});function addOutlineBorder(tableId,columnTitle){if(columnTitle.indexOf(' ')!==-1){columnTitle=columnTitle.substring(0,columnTitle.indexOf(' '))}
