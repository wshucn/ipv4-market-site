...
<?php if ( empty( $required ) ) { ?>
	<p class="uk-margin-remove form-row form-row-wide wc-pao-addon-wrap wc-pao-addon-<?php echo sanitize_title( $field_name ); ?>">
		<label>
			<input type="radio" class="uk-radio wc-pao-addon-field wc-pao-addon-radio" value="" name="addon-<?php echo sanitize_title( $field_name ); ?>[]" />&nbsp;&nbsp;<?php esc_html_e( 'None', 'woocommerce-product-addons' ); ?>
		</label>
	</p>
<?php } ?>
...
<?php
	foreach ( $addon['options'] as $i => $option ) {
		...
		<p class="uk-margin-remove form-row form-row-wide wc-pao-addon-wrap wc-pao-addon-<?php echo sanitize_title( $field_name ); ?>">
			<label>
				<input type="radio" class="uk-radio wc-pao-addon-field wc-pao-addon-radio" name="addon-<?php echo sanitize_title( $field_name ); ?>[]" data-raw-price="<?php echo esc_attr( $price_raw ); ?>" data-price="<?php echo esc_attr( $price_display ); ?>" data-price-type="<?php echo esc_attr( $price_type ); ?>" value="<?php echo sanitize_title( $label ); ?>" <?php checked( $current_value, 1 ); ?> <?php if ( WC_Product_Addons_Helper::is_addon_required( $addon ) ) { echo 'required'; } ?> data-label="<?php echo esc_attr( wptexturize( $label ) ); ?>" />&nbsp;&nbsp;<?php echo wptexturize( $label . ' ' . $price_for_display ); ?>
			</label>
		</p>
<?php } ?>
